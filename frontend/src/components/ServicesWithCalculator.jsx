...
const ServicesWithCalculator = ({ data }) => {
  ...
  // Separate bots for inserting calculator
  const yclientsServices = groupedServices?.yclients || [];
  const firstBot = yclientsServices.find(s => s.id === 1); // Пропущенные звонки
  const secondBot = yclientsServices.find(s => s.id === 2); // Показатели сотрудников  
  const otherBots = yclientsServices.filter(s => s.id !== 1 && s.id !== 2);
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="yclients" className="mt-0">
              <div className="space-y-24">
                {/* First Bot - Пропущенные звонки */}
                {firstBot && <ServiceCard key={firstBot.id} service={firstBot} index={0} />}
                
                {/* ROI Calculator after first bot */}
                <ROICalculator />
                
                {/* Second Bot - Показатели сотрудников */}
                {secondBot && <ServiceCard key={secondBot.id} service={secondBot} index={1} />}
                
                {/* Other bots */}
                {otherBots.map((service, index) => (
...
export default ServicesWithCalculator;